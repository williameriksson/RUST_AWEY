...
fn decode<'a>(n: usize, seed: u32, wordarr: &'a [u32], mut bytearr: &'a mut [u8]) -> (u32, u32) {
    let m;
    let r;
    let (new_seed, mut x) = codgen(seed);
    ...
    if wordarr[n] == 0 {
        ...
        let (y, new_seed) = decode(n + 1, new_seed, &wordarr, &mut bytearr);
        m = x.wrapping_sub(y).wrapping_sub(wordarr[n]);
        bytearr[n] = ((m >> 13) & 0xFF) as u8;
        let tmp = codgen(new_seed);
        r = (!(tmp.1)).wrapping_add(1);
        return (x.wrapping_add(y).wrapping_add(m).wrapping_add(r).wrapping_add(5), tmp.0);
...
fn main() {
    let wordarr: [u32; 132] = [0x015e7a47, 0x2ef84ebb, 0x177a8db4, 0x1b722ff9, 0x5dc7cff0, 0x5dc9dea6, 0x1da0c15a,
    ...
    decode(0, 0x0e0657c1, &wordarr, &mut bytearr);
